...
export default function TableHead<
  T = ColumnDataParameter,
  K = ColumnExternalSourceParameter
>({
  currentSort,
  changeCurrentSort,
  columns,
  ...
  changeCurrentSort:
    | ((sort: TableSortProps<T> | undefined) => void)
    | undefined;
  ...
  const handleChangeSort = (
    ...
    const theSecondSort =
      theFirstSort === SortTypeEnum.DESC ? SortTypeEnum.ASC : SortTypeEnum.DESC;
    if (!isCurrentSort) {
      changeCurrentSort({
        sortBy: columnSortBy,
          ...
          return (
            ...
            >
              <Box
                ...
                sx={{
                  color:
                    column?.sortBy && isCurrentSort ? "neutral1" : "neutral5",
                  "&:hover": {
                ...
                {column.sortBy && changeCurrentSort ? (
                  ...
                  >
                    <Flex
                      alignItems="center"
                      as="span"
                      sx={{ justifyContent: column.style?.textAlign }}
                    >
                      {column.title}
                      {isCurrentSort ? (
